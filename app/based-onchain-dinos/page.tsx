...
const CONTRACT_ADDR = "0x0669583e7d5bE64967153dd276987415926b5e32" as const;
const COLLECTION_ADDR = "0xD4c5292b9689238f0A51C8505B1d1D6714Ce95a0" as const;
...
export default function BasedOnchainDinosPage() {
  ...
  useEffect(() => {
    document.title = "Based Onchain Dinos · flooor.fun — 5% royalty accumulates in the daily vault — 16-hour sign phase, 8-hour claim phase";
  }, []);
  ...
  const handleBid = useCallback(async () => {
    if (isBidding) return;
    if (!IS_DEPLOYED) {
      toast.info("Based Onchain Dinos contract is not live yet — stay tuned.");
      return;
    }
    if (!address) {
    ...
    try {
      ...
      setSharePrompt({
        type: "bid",
        text: `Just placed a bid of Ξ${fmtEth(effectiveBidInput)} on a Based Onchain Dino at flooor.fun 🔨\n\nIf someone outbids me, my ETH comes right back — no risk, no lockup.\n\nRoyalties to the community.`,
      });
      setBidInput("");
      setTimeout(() => {
        getCurrentBid();
  ...
  const handleSellNFT = useCallback(
    async (tokenId: bigint) => {
      ...
      try {
        ...
        setSharePrompt({
          type: "sell",
          text: `Just sold my Based Onchain Dino for Ξ${fmtEth(currentBid)}${soldUsd ? ` (${soldUsd})` : ""} on flooor.fun 🤝\n\nInstant liquidity, any time. Every sale feeds the vault — distributed to holders daily.`,
        });
        setTimeout(() => {
          getCurrentBid();
          getActiveBidder();
  ...
  return (
    ...
    >
      {/* Header */}
      <SiteHeader collectionUrl="https://opensea.io/collection/based-onchain-dinos" soundOn={soundOn} onToggleSound={toggleSound} />
            ...
            <div className="flex items-center justify-center">
              ...
              >
                ...
                <div className="px-3.5 py-3 flex flex-wrap items-center justify-between gap-x-4 gap-y-1.5">
                  ...
                  <a
                    href="https://opensea.io/assets/base/0xD4c5292b9689238f0A51C8505B1d1D6714Ce95a0"
                    target="_blank"
            ...
            {!IS_DEPLOYED ? (
              ...
              >
                ...
                <p style={{ ...SANS, fontSize: "14px", color: MUTED, lineHeight: 1.6 }}>
                  The Based Onchain Dinos contract is being finalized and isn&apos;t live yet. Connect your wallet to be ready when it ships.
                </p>
              </div>
            ) : (
              <div>
