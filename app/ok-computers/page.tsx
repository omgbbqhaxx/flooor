...
import OKCOMPUTERS_ABI from "@/app/abi/okcomputers.json";
...
const CONTRACT_ADDR = "0xE0AB0cE2FF407f53b9384205258C037c22cB03A0" as const;
const COLLECTION_ADDR = "0xCE2830932889C7fB5e5206287C43554E673dCc88" as const;
...
const OKCOMPUTERS_IMG =
  "https://i2c.seadn.io/base/05d807397e5b420d8b9cc7cb8cb07a0d/549fb12b972ea6f3790a2965d31686/55549fb12b972ea6f3790a2965d31686.gif";
...
export default function OkComputersPage() {
  ...
  useEffect(() => {
    document.title = "OK Computers · flooor.fun — 5% royalty accumulates in the daily vault — 16-hour sign phase, 8-hour claim phase";
  }, []);
  ...
  // OK Computers koleksiyonu ERC721Enumerable değil (tokenOfOwnerByIndex yok —
  ...
  const handleBid = useCallback(async () => {
    if (isBidding) return;
    if (!IS_DEPLOYED) {
      toast.info("OK Computers contract is not live yet — stay tuned.");
      return;
    }
    if (!address) {
  ...
  const handleSellNFT = useCallback(
    async (tokenId: bigint) => {
      ...
      try {
        ...
        setSharePrompt({
          type: "sell",
          text: `Just sold my OK Computer for Ξ${fmtEth(currentBid)}${soldUsd ? ` (${soldUsd})` : ""} on flooor.fun 🤝\n\nInstant liquidity, any time. Every sale feeds the vault — distributed to holders daily.`,
        });
        setTimeout(() => {
          getCurrentBid();
          getActiveBidder();
  ...
  return (
    ...
    >
      {/* Header */}
      <SiteHeader collectionUrl="https://opensea.io/assets/base/0xCE2830932889C7fB5e5206287C43554E673dCc88" soundOn={soundOn} onToggleSound={toggleSound} />
            ...
            {!IS_DEPLOYED ? (
              ...
              >
                ...
                <p style={{ ...SANS, fontSize: "14px", color: MUTED, lineHeight: 1.6 }}>
                  The OK Computers contract is being finalized and isn&apos;t live yet. Connect your wallet to be ready when it ships.
                </p>
              </div>
            ) : (
              <div>
