    ...
    // 0x4200000000000000000000000000000000000006 BASE (chainId 8453) WETH — constructor arg (_weth)

    interface IERC721Like {
        function safeTransferFrom(address from, address to, uint256 tokenId) external;
        function transferFrom(address from, address to, uint256 tokenId) external; // <<< EKLENDI (grief fix)
    ...
    contract flooorloopers {
        ...
        address public constant collectionId = 0x1649CD37f4748807b4882FC48765bA0B2aFfa94a;
        ...
        function sellToHighest(uint256 tokenId) external nonReentrant onlyNFTOwnerWrite(tokenId) {
            require(activeBidder != address(0), "No active bid");
        ...
        function setMinBidIncrementPercentage(uint8 p) external onlyOwner {
            require(p >= 1 && p <= 50, "out of range");
            minBidIncrementPercentage = p;
        }


        function setMinBid(uint256 newMin) external onlyOwner {
            require(newMin > 0, "zero min");
            emit MinBidUpdated(minbidAM, newMin);
            minbidAM = newMin;
        }
        ...
        function sweepETH(uint256 amt) external onlyOwner nonReentrant {
        // Borçlar: havuz (claim edecekler) + aktif teklif (escrow)
        uint256 poolLiab = poolAccrued;
        uint256 cur  = currentEpochStart();
        uint256 snap = poolSnap[cur];
        if (snap > 0) {                                   // snap>0 ⇒ partCount[cur]>0 garanti
            uint256 perShare  = snap / partCount[cur];
            uint256 remaining = snap - perShare * claimedCount[cur];
            poolLiab += remaining;
        }

        uint256 liabilities = poolLiab + activebidAM;     // <<< aktif teklif de korunuyor
        uint256 bal     = address(this).balance;
        uint256 surplus = bal > liabilities ? bal - liabilities : 0;
        require(amt <= surplus, "exceeds surplus");        // <<< sadece serbest fazlalık
