...
import ROBINHOOD_ABI from "@/app/abi/stonkbrokers.json";
...
const CONTRACT_ADDR = "0x04ec8eDB287D20f29B834b47a44713d814b0788a" as const;
const COLLECTION_ADDR = "0x539CdD042c2f3d93EbC5BE7DfFf0c79F3B4fAbF0" as const;
...
export default function StonkBrokersPage() {
  const config = useConfig();

  useEffect(() => {
    document.title = "Stonk Brokers · flooor.fun — 5% royalty accumulates in the daily vault — 16-hour sign phase, 8-hour claim phase";
  }, []);
  ...
  const handleBid = useCallback(async () => {
    if (isBidding) return;
    if (!IS_DEPLOYED) {
      toast.info("Stonk Brokers contract is not live yet — stay tuned.");
      return;
    }
    if (!address) {
    ...
    try {
      ...
      setSharePrompt({
        type: "bid",
        text: `Just placed a bid of Ξ${fmtEth(effectiveBidInput)} on Stonk Brokers at flooor.fun/robinhood/stonkbrokers 🔨\n\nIf someone outbids me, my ETH comes right back — no risk, no lockup.\n\nRoyalties to the community.`,
      });
      setBidInput("");
      setTimeout(() => {
        getCurrentBid();
    ...
    async (tokenId: bigint) => {
      ...
      try {
        ...
        if (isSignPhase) {
          ...
          setSharePrompt({
            type: "claim",
            text: `Claimed Ξ${fmtEth(yieldPerSigner)}${claimedUsd ? ` (${claimedUsd})` : ""} from today's vault on flooor.fun/robinhood/stonkbrokers 💰\n\nMy Robinhood NFT earns yield every single day — no lockup, no transfer.`,
          });
        }
        setTimeout(() => {
          checkSignClaimStatus();
  ...
  const handleShare = useCallback(
    async (platform: "x" | "farcaster") => {
      ...
      // hesabı + /flooor kanalı (ayrı token'lar), X'te flooor + Stonk Brokers'un
      // kendi hesabı etiketleniyor (Base değil — Stonk Brokers Robinhood Chain'de)
      const mentions =
        platform === "farcaster"
          ? "@farcaster /flooor"
          : "@vrnouns";
      const text = `${sharePrompt.text}\n\n${mentions}`;
      const url = "https://flooor.fun/robinhood/stonkbrokers";
      setSharePrompt(null);
  ...
  const handleSellNFT = useCallback(
    async (tokenId: bigint) => {
      ...
      try {
        ...
        setSharePrompt({
          type: "sell",
          text: `Just sold my Stonk Brokers NFT for Ξ${fmtEth(currentBid)}${soldUsd ? ` (${soldUsd})` : ""} on flooor.fun/robinhood/stonkbrokers 🤝\n\nInstant liquidity, any time. Every sale feeds the vault — distributed to holders daily.`,
        });
        setTimeout(() => {
          getCurrentBid();
          getActiveBidder();
        ...
        >
          ...
          >
            ...
            <p
              ...
              style={{ color: MUTED }}
            >
              Stonk Brokers runs on Robinhood Chain. Please switch your wallet to
              continue.
            </p>
            <button
              onClick={() => {
              ...
              >
                ...
                >
                  ...
                  <div className="mt-3 flex flex-wrap items-center justify-between gap-x-4 gap-y-1.5 px-1">
                    ...
                    <a
                      href="https://opensea.io/collection/stonkbrokers-434284142"
                      target="_blank"
            ...
            {!IS_DEPLOYED ? (
              ...
              >
                ...
                <p style={{ ...SANS, fontSize: "14px", color: MUTED, lineHeight: 1.6 }}>
                  The Stonk Brokers contract is being finalized and isn&apos;t live yet. Connect your wallet to be ready when it ships.
                </p>
              </div>
            ) : (
              <div>
