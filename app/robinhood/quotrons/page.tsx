...
import ROBINHOOD_ABI from "@/app/abi/quotrons.json";
...
const CONTRACT_ADDR = "0xc347759352F8f2e556CE6A0Ea8048Fb48cfFca45" as const;
const COLLECTION_ADDR = "0x027ACa2794E44f24950D81227DcD516FfBB49d6e" as const;
...
export default function QuotronsPage() {
  const config = useConfig();

  useEffect(() => {
    document.title = "QUOTRONS · flooor.fun — 5% royalty accumulates in the daily vault — 16-hour sign phase, 8-hour claim phase";
  }, []);
  ...
  const getUserNFTs = useCallback(async () => {
    ...
    try {
      const ids = await scanOwnedTokenIds({
        ...
        retryWithBackoff,
        chainId: robinhoodChain.id,
        // QUOTRONS bir ERC404 aynası: totalSupply yok ve tokenId'ler seyrek.
        // Alchemy transfer indeksi tıkanırsa bu aralık taranır.
        fallbackMaxTokenId: 4444,
      });
  ...
  const handleBid = useCallback(async () => {
    if (isBidding) return;
    if (!IS_DEPLOYED) {
      toast.info("QUOTRONS contract is not live yet — stay tuned.");
      return;
    }
    if (!address) {
    ...
    try {
      ...
      setSharePrompt({
        type: "bid",
        text: `Just placed a bid of Ξ${fmtEth(effectiveBidInput)} on QUOTRONS at flooor.fun/robinhood/quotrons 🔨\n\nIf someone outbids me, my ETH comes right back — no risk, no lockup.\n\nRoyalties to the community.`,
      });
      setBidInput("");
      setTimeout(() => {
        getCurrentBid();
    ...
    async (tokenId: bigint) => {
      ...
      try {
        ...
        if (isSignPhase) {
          ...
          setSharePrompt({
            type: "claim",
            text: `Claimed Ξ${fmtEth(yieldPerSigner)}${claimedUsd ? ` (${claimedUsd})` : ""} from today's vault on flooor.fun/robinhood/quotrons 💰\n\nMy Robinhood NFT earns yield every single day — no lockup, no transfer.`,
          });
        }
        setTimeout(() => {
          checkSignClaimStatus();
  ...
  const handleShare = useCallback(
    async (platform: "x" | "farcaster") => {
      ...
      // hesabı + /flooor kanalı (ayrı token'lar), X'te flooor + QUOTRONS'un
      // kendi hesabı etiketleniyor (Base değil — QUOTRONS Robinhood Chain'de)
      const mentions =
        platform === "farcaster"
          ? "@farcaster /flooor"
          : "@vrnouns";
      const text = `${sharePrompt.text}\n\n${mentions}`;
      const url = "https://flooor.fun/robinhood/quotrons";
      setSharePrompt(null);
  ...
  const handleSellNFT = useCallback(
    async (tokenId: bigint) => {
      ...
      try {
        ...
        setSharePrompt({
          type: "sell",
          text: `Just sold my QUOTRONS NFT for Ξ${fmtEth(currentBid)}${soldUsd ? ` (${soldUsd})` : ""} on flooor.fun/robinhood/quotrons 🤝\n\nInstant liquidity, any time. Every sale feeds the vault — distributed to holders daily.`,
        });
        setTimeout(() => {
          getCurrentBid();
          getActiveBidder();
        ...
        >
          ...
          >
            ...
            <p
              ...
              style={{ color: MUTED }}
            >
              QUOTRONS runs on Robinhood Chain. Please switch your wallet to
              continue.
            </p>
            <button
              onClick={() => {
              ...
              >
                ...
                >
                  ...
                  <div className="mt-3 grid grid-cols-2 sm:grid-cols-[1fr_auto_1fr] items-center gap-x-4 gap-y-1.5 px-1">
                    ...
                    <a
                      href="https://opensea.io/collection/quotrons404"
                      target="_blank"
            ...
            {!IS_DEPLOYED ? (
              ...
              >
                ...
                <p style={{ ...SANS, fontSize: "14px", color: MUTED, lineHeight: 1.6 }}>
                  The QUOTRONS contract is being finalized and isn&apos;t live yet. Connect your wallet to be ready when it ships.
                </p>
              </div>
            ) : (
